...
public function showPaymentForm(Fee $fee)
{
    ...
    // store fee_id in session for later use during OTP verification
    session()->put('fee_id', $fee->id);

    return view('roles.student.pay-fee', compact('fee'));
}
    ...
    public function handleGatewayResponse(Request $request)
{
    ...
    $amount = $fee->due_amount; // use fee's due amount as mock payment
    $mockPaymentId = $request->get('paymentId') ?? uniqid('mock_', true);

    $fee->transactions()->create([
        'amount' => $amount,
        'transaction_date' => now(),
        'transaction_id' => $mockPaymentId,
        'payment_method' => 'mock-otp',
        'status' => 'success',
    ]);

    return redirect()->route('student.semesters')->with('success', 'Payment successful (mocked)');
}
