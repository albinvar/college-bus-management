<x-app-layout>
  ...
  <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-8">
    ...
    <div class="bg-white shadow rounded-2xl p-6">
        <h3 class="text-2xl font-bold text-gray-700">Bus #{{ $bus->id }} - Route:  {{ $bus->boardingPoints->first()->place }}  to  {{ $bus->boardingPoints->last()->place }} </h3>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
    <div class="bg-white rounded-2xl shadow p-5">
        ...
        <h2 class="text-2xl font-semibold text-green-700 mt-1">{{ $avg ?? 'N/A' }} ppm</h2>
    </div>
    <div class="bg-white rounded-2xl shadow p-5">
        <p class="text-sm text-gray-500">Peak CO₂</p>
        <h2 class="text-2xl font-semibold text-red-700 mt-1">{{ $peak ?? 'N/A' }} ppm</h2>
    </div>
    <div class="bg-white rounded-2xl shadow p-5">
        <p class="text-sm text-gray-500">Min CO₂</p>
        <h2 class="text-2xl font-semibold text-blue-700 mt-1">{{ $min ?? 'N/A' }} ppm</h2>
    </div>
    <div class="bg-white rounded-2xl shadow p-5">
        <p class="text-sm text-gray-500">Time in High CO₂ Zones</p>
        <h2 class="text-2xl font-semibold text-yellow-700 mt-1">{{ $redZoneTime ?? '0h 0m' }}</h2>
    </div>
</div>
...
<div class="bg-white rounded-2xl shadow p-6 overflow-auto">
    ...
    <table class="min-w-full divide-y divide-gray-200 text-sm">
        ...
        <tbody class="divide-y divide-gray-200">
            @forelse($dailySummaries as $reading)
                <tr>
                    <td class="px-4 py-3">{{ \Carbon\Carbon::parse($reading->reading_date)->format('M d') }}</td>
                    <td class="px-4 py-3">{{ $reading->avg_co2 }} ppm</td>
                    <td class="px-4 py-3">{{ $reading->peak_co2 }} ppm</td>
                    <td class="px-4 py-3">
                        @php
                            $hrs = floor($reading->time_in_red_zone / 60);
                            $min = $reading->time_in_red_zone % 60;
                        @endphp
                        {{ $hrs }}h {{ $min }}m
                    </td>
                    <td class="px-4 py-3">{{ $reading->notes ?? '—' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="5" class="px-4 py-3 text-center text-gray-500">
                        No CO₂ data available for this bus.
                    </td>
                </tr>
            @endforelse
        </tbody>
